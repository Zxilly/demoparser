...
class WantedPropState:
    def __new__(cls, prop: str, state: Union[bool, str, int, float]) -> WantedPropState: ...

@final
class DemoParser:
    def __new__(cls, demo_path: str, *, threads: Optional[int] = None) -> DemoParser:
        """Open a demo and optionally limit parsing to a private worker pool.

        Set ``threads=1`` to disable parallel parsing. When omitted, the parser
        uses its existing automatic strategy and Rayon's global worker pool.
        """
        ...
